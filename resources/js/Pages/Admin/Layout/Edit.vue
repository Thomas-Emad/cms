    ...
    <div class="grid max-w-7xl grid-cols-1 gap-6 xl:grid-cols-[26rem_1fr]">
        ...
        <form class="min-w-0" @submit.prevent="save">
            ...
            <div class="mt-4 rounded-lg border border-slate-200 bg-white p-4">
                <div class="flex items-center justify-between">
                    <p class="text-sm font-semibold text-slate-700">{{ $t('admin.layout.menu_items', 'Menu items') }}
                    </p>
                    <button type="button" class="text-xs text-slate-400 hover:underline" @click="resetItems">{{
                        $t('admin.layout.reset_defaults', 'Reset to defaults') }}</button>
                </div>

                <ul class="mt-2 space-y-2" data-testid="items">
                    <li v-for="(it, i) in form.items" :key="i" class="rounded-md border border-slate-200 p-2.5"
                        :class="{ 'opacity-50': !it.visible }">
                        <div class="flex items-center gap-2">
                            <div class="flex flex-col">
                                <button type="button" class="text-slate-400 hover:text-slate-700 disabled:opacity-20"
                                    :disabled="i === 0" aria-label="Move up" @click="move(i, -1)">▲</button>
                                <button type="button" class="text-slate-400 hover:text-slate-700 disabled:opacity-20"
                                    :disabled="i === form.items.length - 1" aria-label="Move down"
                                    @click="move(i, 1)">▼</button>
                            </div>
                            <input v-model="it.icon" maxlength="4"
                                class="w-11 rounded-md border border-slate-300 px-1 py-1.5 text-center text-lg"
                                :aria-label="`Icon for ${it.label}`" />
                            ...
                            <input v-if="form.template === 'tv'" type="color" :value="it.color ?? '#1f2937'"
                                class="h-8 w-8 shrink-0 rounded" title="Tile colour"
                                @input="it.color = ($event.target as HTMLInputElement).value" />
                            <label class="flex shrink-0 items-center gap-1 text-xs text-slate-500"><input
                                    v-model="it.visible" type="checkbox" />{{ $t('admin.layout.show', 'Show') }}</label>
                            <button type="button" class="shrink-0 text-red-400 hover:text-red-600"
                                aria-label="Delete item" @click="removeItem(i)">✕</button>
                        </div>
                        <input v-model="it.href" placeholder="/facilities"
                            class="mt-1.5 w-full rounded-md border border-slate-300 px-2 py-1 font-mono text-xs"
                            :aria-label="`Page address for ${it.label}`" />
                    </li>
                </ul>
                <button type="button"
                    class="mt-2 w-full rounded-md border border-dashed border-slate-300 py-1.5 text-sm text-slate-500 hover:bg-slate-50"
                    data-testid="add-item" @click="addItem">
                    {{ $t('admin.layout.add_item', '+ Add item') }}
                </button>
            </div>

            <button type="submit" :disabled="form.processing"
                class="mt-4 w-full rounded-md bg-slate-800 px-4 py-2.5 text-sm font-medium text-white hover:bg-slate-900 disabled:opacity-50"
                data-testid="save">
                {{ form.processing ? $t('admin.common.saving', 'Saving…') : $t('admin.layout.save_layout') }}
            </button>
        </form>
        ...
        <div class="min-w-0">
            <div class="mb-2 flex gap-2">
                <button type="button" class="rounded-md border px-3 py-1 text-sm"
                    :class="previewTab === 'home' ? 'border-slate-800 bg-slate-800 text-white' : 'border-slate-300 text-slate-600'"
                    data-testid="preview-home" @click="previewTab = 'home'">
                    {{ $t('admin.layout.preview_home', 'Home screen') }}
                </button>
                <button type="button" class="rounded-md border px-3 py-1 text-sm"
                    :class="previewTab === 'other' ? 'border-slate-800 bg-slate-800 text-white' : 'border-slate-300 text-slate-600'"
                    data-testid="preview-other" @click="previewTab = 'other'">
                    {{ $t('admin.layout.preview_other', 'Another page') }}
                </button>
            </div>
            <div class="overflow-hidden rounded-xl border border-slate-300 bg-slate-900" style="aspect-ratio: 16 / 9"
                data-testid="preview">
                <div class="h-full w-full origin-top-left"
                    style="width: 177.78%; height: 177.78%; transform: scale(0.5625); font-size: 22px">
                    <div :key="previewTab" style="height: 100%">
                        <component :is="previewConfig.template === 'tv' ? TvShell : ClassicShell" :hotel="previewHotel"
                            :config="previewConfig" :preview="true" :data-preview-page="previewTab">
                            <div v-if="previewTab === 'home' && previewConfig.template === 'tv'"
                                class="h-full w-full bg-gradient-to-br from-slate-700 to-slate-900" />
                            <div v-else class="min-h-[140%] bg-white p-10 pt-28 text-slate-400">— page content —
                                <div class="mt-4 h-96 rounded-lg bg-slate-100" />
                            </div>
                        </component>
                    </div>
                </div>
            </div>
            <p class="mt-2 text-xs text-slate-400">{{ $t('admin.layout.preview_note') }}</p>
        </div>
    </div>
</template>

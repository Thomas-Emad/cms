    ...
    <section class="glass flex max-h-full min-h-0 flex-col overflow-hidden rounded-3xl" data-testid="directions-panel" :data-mode="mode">
        ...
        <header v-if="mode === 'choosing' || mode === 'preview'" class="shrink-0 p-4 pb-2">
            <div class="flex items-stretch gap-3">
                <div class="min-w-0 flex-1 space-y-2">
                    <button type="button" class="flex w-full items-center gap-3 rounded-2xl bg-black/5 px-3.5 py-2 text-left active:scale-[0.99]" data-testid="from-field" @click="emit('change-origin')">
                        ...
                        <span class="min-w-0"><span class="block text-xs uppercase tracking-wider text-slate-500">From</span>
                            <span class="block truncate text-base font-medium text-slate-900">{{ origin?.label ?? 'Choose a starting point' }}</span></span>
                    </button>
                ...
                <button type="button" class="self-center h-10 w-10 shrink-0 rounded-full bg-black/5 text-lg text-[#183c2d] active:rotate-180 transition-transform duration-500" :disabled="!origin?.locationId" :class="{ 'opacity-30': !origin?.locationId }" aria-label="Swap start and destination" data-testid="swap" @click="emit('swap')">⇅</button>
            </div>
        </header>
        ...
        <template v-else-if="mode === 'preview'">
                ...
                <template v-else-if="route">
                    ...
                    <ol class="mt-3 space-y-1 pb-2">
                        <li v-for="s in steps" :key="s.index">
                            <button type="button" class="flex w-full items-center gap-4 rounded-2xl px-3 py-2 text-left active:bg-black/5" @click="emit('goto', s.index)">
            ...
            <div class="shrink-0 px-4 pb-4 pt-2">
                ...
                <button v-else type="button" :class="btn" class="w-full border border-[#183c2d]/25 text-[#183c2d]" @click="emit('cancel')">Back</button>
            </div>
        </template>
        ...
        <template v-else-if="mode === 'navigating' && step">
            ...
            <div class="shrink-0 px-4 pb-3 pt-2">
                ...
                <div class="mt-1 flex items-center justify-between">
                    <button type="button" class="h-10 px-3 text-sm font-medium text-[#183c2d] active:scale-95" data-testid="simulate" @click="emit('simulate')">{{ simulating ? '❚❚ Pause walk' : '▶ Simulate walk' }}</button>
                    <button type="button" class="h-10 px-3 text-sm text-slate-500 active:scale-95" data-testid="end" @click="emit('cancel')">End</button>
                </div>
            </div>
        </template>
        ...
        <template v-else-if="mode === 'arrived'">
            <div class="min-h-0 flex-1 overflow-y-auto overscroll-contain p-5 pb-2 text-center" data-testid="arrived">
                ...
                <h2 class="mt-2 text-3xl text-slate-900" style="font-family: var(--font-display)">You've arrived</h2>

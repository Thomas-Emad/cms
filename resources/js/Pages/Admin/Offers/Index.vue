            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-slate-100">
                    <tr v-for="offer in offers.data" :key="offer.id">
                        <td class="px-4 py-2 font-medium text-slate-700">{{ offer.title }} <span v-if="offer.featured"
                                class="ms-1 text-amber-500">★</span></td>
                        <td class="px-4 py-2 text-slate-500">{{ offer.discount ? offer.discount + '%' : '—' }}</td>
                        <td class="px-4 py-2">
                            <span class="rounded-full px-2 py-0.5 text-xs"
                                :class="offer.status === 'published' ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-500'">
                                {{ $t(`admin.status.${offer.status}`, offer.status) }}
                            </span>
                        </td>
                        <td class="px-4 py-2 text-end space-x-3 rtl:space-x-reverse">
                            <Link :href="`/admin/offers/${offer.id}/edit`" class="text-slate-500 hover:text-slate-800">
                                {{ $t('common.edit') }}</Link>
                            <button class="text-red-500 hover:text-red-700" @click="destroy(offer)">{{

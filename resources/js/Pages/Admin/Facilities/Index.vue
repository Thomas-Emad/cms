...
<template>
    <div>
        <div class="flex items-center justify-between mb-4">
            ...
            <Link :href="createUrl" class="rounded-md bg-slate-800 px-3 py-1.5 text-sm text-white hover:bg-slate-900">
                {{ isMeeting ? t('admin.facilities.add_meeting_room', '+ Add Meeting Room') :
                    t('admin.facilities.add_facility', '+ Add Facility') }}
            </Link>
        </div>
                ...
                <tbody class="divide-y divide-slate-100">
                    <tr v-for="facility in facilities.data" :key="facility.id">
                        ...
                        <td class="px-4 py-2 text-slate-500">
                            {{ $t(`admin.facility_categories.${facility.category}`, facility.category) }}
                        </td>
                        <td class="px-4 py-2">
                        ...
                        <td class="px-4 py-2 text-end space-x-3 rtl:space-x-reverse">
                            ...
                            <button class="text-red-500 hover:text-red-700" @click="destroy(facility)">{{

...
<template>
  <div>
    <div class="flex items-center justify-between mb-4">
      ...
      <Link href="/admin/experiences/create" class="rounded-md bg-slate-800 px-3 py-1.5 text-sm text-white hover:bg-slate-900">
      ...
      <table class="w-full text-sm">
        ...
        <tbody class="divide-y divide-slate-100">
          <tr v-for="experience in experiences.data" :key="experience.id">
            <td class="px-4 py-2 font-medium text-slate-700">{{ experience.title }} <span v-if="experience.featured" class="ms-1 text-amber-500">★</span></td>
            <td class="px-4 py-2 text-slate-500">{{ experience.category }}</td>
            <td class="px-4 py-2">
              <span class="rounded-full px-2 py-0.5 text-xs" :class="experience.status === 'published' ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-500'">
                {{ $t(`admin.status.${experience.status}`, experience.status) }}
              </span>
            </td>
            <td class="px-4 py-2 text-end space-x-3 rtl:space-x-reverse">
              <Link :href="`/admin/experiences/${experience.id}/edit`" class="text-slate-500 hover:text-slate-800">{{ $t('common.edit') }}</Link>
              <button class="text-red-500 hover:text-red-700" @click="destroy(experience)">{{ $t('common.delete') }}</button>
            </td>
          </tr>
        </tbody>
      </table>

...
$pdo->exec("SET FOREIGN_KEY_CHECKS=0"); $pdo->exec("DELETE FROM facilities"); $pdo->exec("SET FOREIGN_KEY_CHECKS=1");
...
function resolveFacilityGridForPage(PDO $pdo, int $pageId, int $hotelId): array
{
    ...
    $params = [$hotelId];
    if (!empty($fgProps['category'])) { $sql .= " AND category = ?"; $params[] = $fgProps['category']; }

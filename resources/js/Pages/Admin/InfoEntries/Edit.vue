...
<template>
  <div class="max-w-4xl">
    ...
    <form @submit.prevent="submit" class="mt-4 rounded-lg border border-slate-200 bg-white p-4">
      ...
      <datalist id="known-groups-ar"><option v-for="g in groupsAr" :key="g" :value="g" /></datalist>
      ...
      <div v-if="activeTab === 'en'">
        ...
        <p v-else class="py-6 text-center text-sm text-slate-400">{{ t('admin.info_entries.empty', 'Nothing here yet — add your first row.') }}</p>
      </div>
      ...
      <div v-if="activeTab === 'ar'" dir="rtl" class="space-y-2">
        ...
        <div v-if="form.entries.length" class="space-y-2">
          <div v-for="(entry, i) in form.entries" :key="entry.id ?? `new-${i}`" class="flex items-start gap-2">
            <input v-if="has_group" v-model="entry.group_ar" list="known-groups-ar" type="text" placeholder="المجموعة (بالعربية)" :class="[input, 'w-40']" />
            <div class="flex-1">
              <input v-model="entry.label_ar" type="text" :placeholder="`الاسم بالعربية (EN: ${entry.label || '—'})`" :class="input" />
            </div>
            <div class="w-56">
              <input v-model="entry.value_ar" type="text" :placeholder="`القيمة بالعربية (EN: ${entry.value || '—'})`" :class="input" />
            </div>
      ...
      <div class="mt-4 flex items-center justify-between border-t border-slate-100 pt-4">
        <button type="button" class="rounded-md border border-slate-300 px-3 py-1.5 text-sm text-slate-700 hover:bg-slate-50" @click="add">
          {{ t('admin.info_entries.add_row', '+ Add row') }}
        </button>

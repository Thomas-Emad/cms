...
// Today
const today = computed<WeatherDay | null>(() => {
    ...
    return props.weather.days.find((day) => day.date === date) ?? null;
});
    ...
    <div class="weather-page min-h-screen">
        <div class="mx-auto max-w-7xl px-6 lg:px-10 pt-24 lg:pt-28 pb-16 lg:pb-24">
            ...
            <div class="mb-8 lg:mb-10">
                <div class="flex flex-wrap items-center justify-between gap-4">
                    <div>
                        <div class="flex items-center gap-2 mb-2">
                            ...
                            <span class="text-xs uppercase tracking-[0.2em] font-medium"
                                style="color: var(--luxury-champagne, #c5a880)">
                                {{ t('weather.five_day_history', undefined, 'Past 5 Days Weather') }}
                            </span>
                        </div>
                        <h1 class="text-3xl lg:text-5xl tracking-tight text-slate-900"
                            style="font-family: var(--font-display)">
                            {{ weather.location?.branch_name ? `${weather.location.branch_name} — ` :
                                (weather.location?.city ? `${weather.location.city} — ` : '') }}{{ t('weather.title',
                                undefined, 'Weather') }}
                        </h1>
                    ...
                    <div v-if="weather.location?.city"
                ...
                <div v-if="weather.branches && weather.branches.length > 1"
                    ...
                    <div class="no-scrollbar flex items-center gap-2.5 overflow-x-auto pb-1">
                        <button v-for="branch in weather.branches" :key="branch.id" type="button"
                            ...
                            <span v-if="branch.id === weather.current_branch_id" class="text-xs opacity-75 ps-1">
                                ✓
                            </span>
                        </button>
                    </div>
            ...
            <!-- Error State -->
            <div v-if="weather.status === 'error' || !hasDays"
                class="rounded-2xl border border-red-100 bg-red-50/60 p-8 lg:p-12 text-center max-w-xl mx-auto shadow-sm">
                <div
                    class="mx-auto w-14 h-14 rounded-full bg-red-100 text-red-600 flex items-center justify-center text-2xl mb-4">
                    ⚠️
                </div>
                ...
                <p class="text-sm lg:text-base text-slate-600 mb-6">
                    {{ t('weather.error_message', undefined) }}
                </p>
                <div class="flex items-center justify-center gap-3">
                    <button type="button" :disabled="isRetrying"
                        ...
                        style="background: var(--color-primary, #1f4b5a)" @click="retry">
                        {{ isRetrying ? t('weather.loading', undefined, 'Loading…') : t('weather.retry', undefined) }}
                    </button>
                    <Link href="/"
            ...
            <div v-else class="space-y-8 lg:space-y-12 transition-opacity duration-200"
                :class="{ 'opacity-60 pointer-events-none': isSwitchingBranch }">
                <!-- Featured / Most Recent Day Banner -->
                <div v-if="today" class="relative overflow-hidden rounded-3xl p-6 lg:p-10 shadow-lg text-white"
                    style="background: linear-gradient(135deg, var(--color-primary, #0f2d3a) 0%, #1a4454 100%)">
                    <!-- Background ambient graphic -->
                    <div class="pointer-events-none absolute -end-16 -top-16 h-72 w-72 rounded-full opacity-15 blur-2xl"
                        style="background: var(--luxury-champagne, #c5a880)" aria-hidden="true" />

                    <div class="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
                        <!-- Main Day Information -->
                        <div class="lg:col-span-7">
                            <div class="flex flex-wrap items-center gap-3 mb-3">
                                <span class="px-3 py-1 rounded-full text-xs uppercase tracking-wider font-semibold"
                                    style="background: rgba(255, 255, 255, 0.18); color: #ffffff">
                                    {{ t('weather.latest_observation', undefined, 'Most Recent Day') }}
                                </span>
                                <span v-if="weather.location?.branch_name"
                                    class="px-2.5 py-0.5 rounded-full text-xs text-white/90 bg-white/10">
                                    {{ weather.location.branch_name }}
                                </span>
                                <span class="text-sm text-white/70">{{ formatLocalizedDate(today.date) }}</span>
                            </div>

                            <div class="flex flex-wrap items-baseline gap-4 mt-2">
                                <span class="text-6xl lg:text-8xl font-light tracking-tighter tabular-nums"
                                    style="font-family: var(--font-display)">
                                    {{ Math.round(today.temp_max) }}°
                                </span>
                                <div class="flex flex-col">
                                    <span class="text-2xl lg:text-3xl font-medium text-white/95">
                                        {{ getConditionLabel(today.condition_key) }}
                                    </span>
                                    <span class="text-sm text-white/75 mt-0.5">
                                        {{ t('weather.high') }}: {{ today.temp_max }}°{{ weather.units.temperature }} ·
                                        {{ t('weather.low') }}: {{ today.temp_min }}°{{ weather.units.temperature }}
                                    </span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div
                            class="lg:col-span-5 grid grid-cols-2 gap-3 sm:gap-4 border-t lg:border-t-0 lg:border-s border-white/15 pt-6 lg:pt-0 lg:ps-8">
                            <div class="rounded-2xl p-4 bg-white/10 backdrop-blur-md">
                                <span class="block text-xs uppercase tracking-wider text-white/60 mb-1">{{
                                    t('weather.feels_like') }}</span>
                                <span class="text-xl lg:text-2xl font-semibold tabular-nums">{{ today.apparent_temp_max
                                    }}°{{ weather.units.temperature }}</span>
                            </div>
                            <div class="rounded-2xl p-4 bg-white/10 backdrop-blur-md">
                                <span class="block text-xs uppercase tracking-wider text-white/60 mb-1">{{
                                    t('weather.high_low') }}</span>
                                <span class="text-xl lg:text-2xl font-semibold tabular-nums">{{ today.temp_max }}° / {{
                                    today.temp_min }}°</span>
                            </div>
                            <div class="rounded-2xl p-4 bg-white/10 backdrop-blur-md">
                                <span class="block text-xs uppercase tracking-wider text-white/60 mb-1">{{
                                    t('weather.precipitation') }}</span>
                                <span class="text-xl lg:text-2xl font-semibold tabular-nums">{{ today.precipitation }}
                                    {{ weather.units.precipitation }}</span>
                            </div>
                            <div class="rounded-2xl p-4 bg-white/10 backdrop-blur-md">
                                <span class="block text-xs uppercase tracking-wider text-white/60 mb-1">{{
                                    t('weather.wind') }}</span>
                                <span class="text-xl lg:text-2xl font-semibold tabular-nums">{{ today.wind_speed }} {{
                                    weather.units.wind }}</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div>
                    <div class="flex items-center justify-between mb-6">
                        <div>
                            ...
                            <p class="text-xs lg:text-sm text-slate-400 mt-1">
                                {{ t('weather.historical_notice', undefined) }}
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 lg:gap-5">
                        <div v-for="day in weather.days" :key="day.date"
                            ...
                            :class="{
                                'ring-2 ring-emerald-500/30 border-emerald-400/60 bg-emerald-50/20': day.date === today?.date,
                            }">
                            ...
                            <div class="my-3 text-center">
                                <!-- Weather Icon Symbol -->
                                <div
                                    class="inline-flex items-center justify-center w-14 h-14 rounded-2xl bg-slate-50 text-3xl group-hover:scale-110 transition-transform duration-200">
                                ...
                                <p
                                    class="mt-2 text-sm font-medium text-slate-700 min-h-[2.5rem] flex items-center justify-center leading-snug">

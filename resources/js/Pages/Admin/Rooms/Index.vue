        ...
        <tbody class="divide-y divide-slate-100">
          <tr v-for="room in rooms.data" :key="room.id">
            ...
            <td class="px-4 py-2 text-end space-x-3 rtl:space-x-reverse">
              <Link :href="`/admin/rooms/${room.id}/edit`" class="text-slate-500 hover:text-slate-800">{{ $t('common.edit') }}</Link>
              <button class="text-red-500 hover:text-red-700" @click="destroy(room)">{{ $t('common.delete') }}</button>
            </td>
          </tr>
        </tbody>
      </table>

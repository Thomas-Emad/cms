  ...
  <div class="w-72 shrink-0 border-l border-slate-200 bg-white p-4 overflow-y-auto">
    ...
    <div v-else>
      ...
      <div v-for="field in entry?.editorFields ?? []" :key="field.key" class="mb-3">
        ...
        <!--
          Generic field type, not gallery-specific: any future section
          needing a list of media references uses this same type. Comma-
          separated numeric IDs are an intentional placeholder (matching
          the single 'media' field's number-input placeholder) until a
          real media picker exists - both are explicitly out of scope
          this checkpoint.
        -->
        <input
          v-else-if="field.type === 'media-list'"
          :value="(section.props[field.key] as number[] | undefined)?.join(', ') ?? ''"
          type="text"
          placeholder="Media IDs, comma-separated (picker UI not built yet)"
          class="w-full rounded-md border border-slate-300 px-2 py-1.5 text-sm"
          @input="onDiscreteFieldInput(
            field.key,

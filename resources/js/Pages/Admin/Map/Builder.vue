...
const hint = computed(() => (b.doorPick.value ? 'Click the walkway point that is this place\'s door.' : TOOLS.find((t) => t.id === b.tool.value)!.hint));
...
<template>
  <div class="flex flex-col overflow-hidden rounded-lg border border-slate-200 bg-white" style="height: calc(100vh - 6.5rem)" data-testid="builder">
    ...
    <div class="flex flex-wrap items-center gap-2 border-b border-slate-200 px-3 py-2">
      <h1 class="mr-2 text-base font-semibold text-slate-800">Map builder</h1>
      <button :class="btn" :disabled="!b.canUndo.value" title="Undo (Ctrl+Z)" data-testid="undo" @click="b.undo()">↶ Undo</button>
      <button :class="btn" :disabled="!b.canRedo.value" title="Redo (Ctrl+Shift+Z)" data-testid="redo" @click="b.redo()">↷ Redo</button>
      <span class="mx-1 h-5 w-px bg-slate-200" />
      <label class="flex items-center gap-1.5 text-sm text-slate-600"><input v-model="b.snapOn.value" type="checkbox" /> Snap to grid</label>
      <label class="flex items-center gap-1.5 text-sm text-slate-600"><input v-model="b.ortho.value" type="checkbox" /> Straight walkways</label>
      <span class="mx-1 h-5 w-px bg-slate-200" />
      <button :class="btn" aria-label="Zoom in" @click="camera.zoomBy(1.4)">＋</button>
      <button :class="btn" aria-label="Zoom out" @click="camera.zoomBy(1 / 1.4)">－</button>
      <button :class="btn" @click="fitFloor(false)">Fit</button>

      <div class="ml-auto flex items-center gap-3">
        <span v-if="flash_ok && !b.dirty.value" class="text-sm text-emerald-600" data-testid="saved">✓ {{ flash_ok }}</span>
        <span v-else-if="b.dirty.value" class="text-sm text-amber-600" data-testid="dirty">Unsaved changes</span>
        <a href="/map" target="_blank" class="text-sm text-sky-700 underline">Guest view</a>
        <Link href="/admin/map" class="text-sm text-slate-400 underline">Advanced (JSON)</Link>
        <button
    ...
    <div v-if="errors_list.length" class="border-b border-red-200 bg-red-50 px-4 py-2 text-sm text-red-700" data-testid="server-errors">
      <b>The map was not saved:</b> <span v-for="(e, i) in errors_list" :key="i">{{ e }} </span>
    </div>
    <div v-else-if="warnings.length && !b.dirty.value" class="border-b border-amber-200 bg-amber-50 px-4 py-2 text-sm text-amber-800">
      <b>Saved, but please check:</b> <span v-for="(w, i) in warnings" :key="i">{{ w }} </span>
    </div>

    <div class="flex min-h-0 flex-1">
      <!-- ============ left: floors + tools ============ -->
      <aside class="w-56 shrink-0 overflow-y-auto border-r border-slate-200 p-3">
        <p class="mb-1.5 text-xs font-semibold uppercase tracking-wide text-slate-400">Tools</p>
        <div class="grid grid-cols-2 gap-1.5">
          <button
            ...
            @click="setTool(t.id)"
          >
            <span class="text-lg leading-none">{{ t.icon }}</span>{{ t.label }}
          </button>
        </div>

        <div v-if="b.tool.value === 'area'" class="mt-3">
          <label class="mb-1 block text-xs text-slate-500">Drawing a…</label>
          <select v-model="b.areaKind.value" :class="sel" data-testid="area-kind-pick">
            <option v-for="k in (Object.keys(AREA_STYLE) as AreaKind[])" :key="k" :value="k">{{ { building: 'Building outline', corridor: 'Corridor / hallway', room: 'Room', public: 'Public space', service: 'Service / staff only', water: 'Pool / water', outdoor: 'Outdoor' }[k] }}</option>
          </select>
        </div>
        <div v-if="b.tool.value === 'location'" class="mt-3">
          <label class="mb-1 block text-xs text-slate-500">Adding a…</label>
          <select v-model="b.placeCategory.value" :class="sel">
            <option v-for="(m, c) in CATEGORY_META" :key="c" :value="(c as LocationCategory)">{{ m.icon }} {{ m.label }}</option>
          </select>
        </div>
        <div v-if="b.tool.value === 'path'" class="mt-3">
          <label class="mb-1 block text-xs text-slate-500">New points are…</label>
          <select v-model="b.pathNodeType.value" :class="sel">
            <option v-for="(l, t) in ({ walk: 'Walkway points', entrance: 'Entrances', elevator: 'Elevators', stairs: 'Stairs' } as Record<NodeType, string>)" :key="t" :value="t">{{ l }}</option>
          </select>
          <button v-if="b.pathFrom.value" :class="[btn, 'mt-2 w-full']" data-testid="path-done" @click="b.pathFrom.value = null">Finish this walkway</button>
        </div>

        <p class="mb-1.5 mt-5 text-xs font-semibold uppercase tracking-wide text-slate-400">Floors</p>
        ...
        <button v-if="!adding" :class="[btn, 'mt-2 w-full']" data-testid="add-floor" @click="openAdd">+ Add floor</button>
        <form v-else class="mt-2 space-y-2 rounded-md border border-slate-200 p-2" @submit.prevent="addFloor">
          <div class="grid grid-cols-[3.5rem_1fr] gap-2">
            <input v-model="newFloor.label" placeholder="Short" maxlength="4" :class="sel" data-testid="nf-label" />
            <input v-model="newFloor.name" placeholder="Full name" :class="sel" />
          </div>
          <input v-model.number="newFloor.level" type="number" :class="sel" title="Level" />
          <select v-model="newFloor.copyFrom" :class="sel" data-testid="nf-copy">
            <option value="">Start empty</option>
            <option v-for="f in floorsTopDown" :key="f.id" :value="f.id">Copy layout of {{ f.name }}</option>
          </select>
          <p class="text-xs text-slate-400">Copying keeps rooms, corridors, lifts and stairs (places are not copied).</p>
          <div class="flex gap-2"><button type="submit" class="flex-1 rounded-md bg-slate-800 px-2 py-1.5 text-sm text-white" data-testid="nf-submit">Add</button><button type="button" :class="btn" @click="adding = false">Cancel</button></div>
        </form>
        <button v-if="b.floors.value.length > 1" class="mt-2 w-full text-left text-xs text-red-500 hover:underline" data-testid="delete-floor" @click="removeFloor">Delete this floor…</button>
      </aside>
      ...
      <div class="max-h-40 overflow-y-auto px-3 pb-2 pt-1.5 text-sm">
        ...
        <template v-else>
          <div class="flex flex-wrap items-center gap-2">
            <select v-model="from" :class="[sel, 'w-56']" data-testid="route-from"><option value="">From…</option><option v-for="l in places" :key="l.id" :value="l.id">{{ l.name }} ({{ b.floorOf(l.floor)?.label }})</option></select>
            <span class="text-slate-400">→</span>
            <select v-model="to" :class="[sel, 'w-56']" data-testid="route-to"><option value="">To…</option><option v-for="l in places" :key="l.id" :value="l.id">{{ l.name }} ({{ b.floorOf(l.floor)?.label }})</option></select>
            <button :class="btn" :disabled="!from || !to || from === to" data-testid="route-go" @click="runTest">Show route</button>
          </div>
          <p v-if="routeTried && !route" class="mt-2 text-red-600" data-testid="route-none">No walking route between those places. Check the walkways and lifts connect.</p>
          <div v-if="route" class="mt-2" data-testid="route-result">
            <p class="font-medium text-slate-800">{{ formatDistance(route.meters) || '0 m' }} · {{ formatDuration(route.seconds) }} <span class="font-normal text-slate-400">(dotted green line on the map)</span></p>
